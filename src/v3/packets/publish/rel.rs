...
impl<'a> Display for PubRel<'a> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "{} pkid({})", ControlPacketType::PubRel, self.pkid)
    }
}

impl<'a> Input<NonZero<u16>> for PubRel<'a> {
    type Validated = ();
...
impl<'a> Parsed for PubRel<'a> {
    fn validate(&self) -> Result<(), crate::bytes::Error> {
        Ok(())
    }
}

impl<'a> Encode<NonZero<u16>> for PubRel<'a> {
    ...
    where
        W: std::io::Write,
    {
        let mut written = FixedHeaderBuilder::new(ControlPacketType::PubRel)
...
mod tests {
    ...
    fn pubrec_roundtrip() {
        ...
        let written = PubRel::write_sync(&mut buf, &pkid).unwrap();
        assert_eq!(written, buf.len());

        let res = PubRel::consume(&buf).unwrap();
