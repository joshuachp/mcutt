...
#[derive(Debug)]
pub struct UnsubscribeBuilder<T> {
...
impl<T> UnsubscribeBuilder<T> {
    ...
    pub fn new<S>(pkid: NonZero<u16>, topic_filters: T) -> Self
    where
...
impl<'a> UnsubscribeBuilder<[&'a str; 1]> {
    /// Creates an unsubscribe with a single topic
    pub fn with_topic(pkid: NonZero<u16>, topic: &'a str) -> Self {
        Self::new(pkid, [topic])
    }
}
